# 必要なモジュールのインポート
import torch
from usamimi import transform, Net # usamimi.py から前処理とネットワークの定義を読み込み
...
def predict(img):
    # ネットワークの準備
    net = Net().cpu().eval()
    # # 学習済みモデルの重み（rabit_ears.pt）を読み込み
    net.load_state_dict(torch.load('src/animal_ears.pt', map_location=torch.device('cpu')))
    # データの前処理
    img = transform(img)
